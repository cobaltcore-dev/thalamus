...
CRD_REF_DOCS   ?= $(shell command -v crd-ref-docs 2>/dev/null || echo $(shell go env GOPATH)/bin/crd-ref-docs)
...
install-crd-ref-docs:
	@if [ ! -x "$(CRD_REF_DOCS)" ]; then \
	    echo "Downloading crd-ref-docs $(CRD_REF_DOCS_VERSION)"; \
...
build-all: build/operator

build/operator: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/operator ./cmd/operator
...
install: FORCE build/operator
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/operator "$(DESTDIR)$(PREFIX)/bin/operator"
...
run-shellcheck: FORCE install-shellcheck
	...
	@find . \( -path '*/website/node_modules/*' -prune \) -o \( -path 'website/node_modules' -prune \) -o -type f \( -name '*.bash' -o -name '*.ksh' -o -name '*.zsh' -o -name '*.sh' -o -name '*.shlib' \) -exec shellcheck  {} +
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/cobaltcore-dev/thalamus\e[0m\n"
	@goimports -w -local github.com/cobaltcore-dev/thalamus $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/operator\e[0m               Build operator.\n"
