...
* Good, because building the abstraction ourselves gives SAP full ownership of features, roadmap, and timeline: no need to merge the major features and breaking changes into external projects or work around them.
* Good, because it builds on and continues prior work in the Thalamus operator.
* Good, because it will be operationally simple for our team, since we only build features we need and reuse SCI solutions, such as Greenhouse.
* Good, because it has a genuine multi-vendor contribution base — over the last 6 months commits split roughly evenly across Google, IBM, and Red Hat (with Intel, Neural Magic, and others), so no single vendor steers it.[^llmd-stats]
* Good, because it is engine-agnostic, so it fits large-scale, production, and air-gapped deployments.
* Neutral, because if Thalamus does not support additional mature backends, a community around the in-house solution should be built, and the decision not to reuse existing solutions should be reasonably justified.
...
* (D1.3) Neutral, because it has no native model catalogue, though one might be translatable into `LLMInferenceServiceConfig` resources from our controller.
* (D1.2) Bad, because the stack is very complex, non-trivial to operate, and debug.
* (D3.2, D3.3) Bad, because the `LLMInferenceService` path that Thalamus depends on is only ~13 months old (first commit July 2025) and ~87% Red Hat-authored (133/153 commits), with no other sustaining vendor.[^kserve-llmisvc-stats]
* (D3.3) Bad, because PR/issue feedback is almost non-existent, so external contributions are hard to get reviewed/merged.
...
* (D3.3) Good, because it has a genuine multi-vendor contribution base — over the last 6 months commits split roughly evenly across Google, IBM, and Red Hat (with Intel, Neural Magic, and others), so no single vendor steers it.[^llmd-stats]
...
* (D1.3) Good, because it ships a preconfigured model catalogue that we could extend.
...
* (D3.1) Neutral, because governance is Microsoft-controlled — both maintainers and the sole CODEOWNERS entry are Microsoft.
...
* (D2.1, D2.2) Bad, because in practice it is NVIDIA/CUDA-only: the controller hardcodes the `accelerator: nvidia` node label and `nvidia.com/gpu` capacity with no ROCm/AMD path, and runtimes are limited to vLLM and HuggingFace Transformers.
...
* (D3.1) Bad, because it is currently led by Upbound and has not yet been donated to a neutral foundation, so governance is single-company.
* (D3.2) Bad, because it is very early (first commit March 2026, currently v0.2.0) and has not proven to be production-ready.
...
* (D2.1) Bad, because in practice only NVIDIA GPUs are supported, despite its "vendor-agnostic" claims.
* (D3.1, D3.3) Bad, because it is vendor-led by NVIDIA with no independent foundation governance or multi-vendor steering.
...
source paths `cmd/llmisvc/`, `pkg/controller/v1alpha2/llmisvc/`, and `pkg/webhook/admission/llminferenceservice/`:
    ...
    # contributors, and Red Hat share via Signed-off-by (resolves gmail/noreply to real affiliation)
    git shortlog -sne HEAD -- "${LLMISVC[@]}"
    git log --format='%an%x09%(trailers:key=Signed-off-by,valueonly)' -- "${LLMISVC[@]}" \
      | awk -F'\t' '$2 ~ /redhat\.com/ || $1 ~ /Majsak/ {c++} END{print c}'  # 133 (Bartosz Majsak is Red Hat)
    ```

    Result: 133/153 commits (~87%) are Red Hat-authored; the top contributor alone (Bartosz Majsak, RH) accounts for
    54 commits (~35%), and the top 5 (all RH) for ~75%. The "2,292 contributors / 627 organizations" headline reflects
    KServe's predictive-ML history, not this path.
...
[llm-d/llm-d](https://github.com/llm-d/llm-d) (`main`, HEAD `44db58f`, 2026-07-17):
    ...
    # vendor split via Signed-off-by, last 6 months (resolves *.ibm.com -> ibm.com, drops bots/noreply)
    ...
    confirming a sustained multi-vendor base rather than single-vendor steering.
...
[kaito-project/kaito](https://github.com/kaito-project/kaito) (`main`, HEAD `56a7731b`, 2026-07-16):
    ...
    ```sh
    # governance: both maintainers and the sole CODEOWNERS entry are Microsoft (Fei-Guo, zhuangqh)
    ...
    Result: governance is Microsoft-controlled (both maintainers + CODEOWNERS), but the last-6-month contribution mix is
    ~114/249 (~46%) Microsoft with a substantial non-Microsoft tail — a genuinely mixed contribution base. Project started
    2023-07-27 (1,506 commits total).

    The NVIDIA/CUDA-only finding is from the controller code, not docs: node readiness hardcodes the `accelerator: nvidia`
